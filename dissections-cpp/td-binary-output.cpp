...
// We accumulate signatures here until we reach some limit, and then we write
// them out.
set<vector<flist> > signature_buffer;

void print_list_of_4lists_binary(vector<flist> lists)
{
    // To look at this output use od:
    //
    // $ od -t dI <signature file>

    int length = 4*2*lists.size();
    assert(fwrite(&length, sizeof(int), 1, stdout) == 1);

    for(vector<flist>::iterator iter = lists.begin(); iter != lists.end(); iter++) {
        assert(iter->a.numerator   >= 0);
        assert(iter->a.denominator >= 0);
        assert(iter->b.numerator   >= 0);
        assert(iter->b.denominator >= 0);
        assert(iter->c.numerator   >= 0);
        assert(iter->c.denominator >= 0);
        assert(iter->d.numerator   >= 0);
        assert(iter->d.denominator >= 0);

        assert(iter->a.numerator   <= 255);
        assert(iter->a.denominator <= 255);
        assert(iter->b.numerator   <= 255);
        assert(iter->b.denominator <= 255);
        assert(iter->c.numerator   <= 255);
        assert(iter->c.denominator <= 255);
        assert(iter->d.numerator   <= 255);
        assert(iter->d.denominator <= 255);

        unsigned char a_numerator   = (unsigned char) iter->a.numerator;
        unsigned char a_denominator = (unsigned char) iter->a.denominator;
        assert(fwrite(&a_numerator,   sizeof(unsigned char), 1, stdout) == 1);
        assert(fwrite(&a_denominator, sizeof(unsigned char), 1, stdout) == 1);

        unsigned char b_numerator   = (unsigned char) iter->b.numerator;
        unsigned char b_denominator = (unsigned char) iter->b.denominator;
        assert(fwrite(&b_numerator,   sizeof(unsigned char), 1, stdout) == 1);
        assert(fwrite(&b_denominator, sizeof(unsigned char), 1, stdout) == 1);

        unsigned char c_numerator   = (unsigned char) iter->c.numerator;
        unsigned char c_denominator = (unsigned char) iter->c.denominator;
        assert(fwrite(&c_numerator,   sizeof(unsigned char), 1, stdout) == 1);
        assert(fwrite(&c_denominator, sizeof(unsigned char), 1, stdout) == 1);

        unsigned char d_numerator   = (unsigned char) iter->d.numerator;
        unsigned char d_denominator = (unsigned char) iter->d.denominator;
        assert(fwrite(&d_numerator,   sizeof(unsigned char), 1, stdout) == 1);
        assert(fwrite(&d_denominator, sizeof(unsigned char), 1, stdout) == 1);
    }
}

void dump_buffer()
{
    for(set<vector<flist> >::iterator sig = signature_buffer.begin(); sig != signature_buffer.end(); sig++) {
        // print_list_of_4lists(*sig);
        print_list_of_4lists_binary(*sig);
    }

    signature_buffer.clear();
}

void send_4list(vector<flist> lists)
{
    signature_buffer.insert(lists);

    if (signature_buffer.size() == 10) {
        dump_buffer();
    }
}
...
int main()
{
    ...
    while(read_bitrade(nr_rows, nr_cols, nr_syms, nr_elements, T1, T2)) {
        // Enumerate all signatures for (T1, T2)
        for(vector<Triple>::iterator tIter = T1.begin(); tIter != T1.end(); tIter++) {
        ...
        // Enumerate all signatures for (T2, T1)
        for(vector<Triple>::iterator tIter = T2.begin(); tIter != T2.end(); tIter++) {

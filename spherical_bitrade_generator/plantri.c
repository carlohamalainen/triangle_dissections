#define VERSION "4.1 - October 3 2004"
#define SWITCHES "[-uagsh -c#txm#P#bpe#f#q -odG -v]"
#define TMP
...
Change History:
       ...
       11-Mar-2008 : Fixed definition of CLK_TCK (Carlo Hamalainen)
...
/* /usr/include/time.h says that CLK_TCK is the 
 * obsolete POSIX.1-1988 name for the same constant, referring to
 * CLOCKS_PER_SEC. So here we just paste in some of the commented code
 * from time.h: */
#ifndef CLK_TCK
...
#define AMADDARC(i,j) am2[i][j] = 1;
#define AMDELARC(i,j) am2[i][j] = 0;
...
static bigint numrooted;  /* rooted maps (fails for connectivity 1) */
...
#endif
#ifdef STATS2   /* even more statistics collection */
    ...
    for (b = 1; b < nv-1; ++b)
    {
        ...
        do
        {
            if (e->end > b && degree[e->end] >= 4)
            {
                ...
                ec = eclast->next->next;
                eclast = eclast->prev;
                for (; ec != eclast; ec = ec->next) if (ISMARKED_V(ec->end)) ++nt;
            }
            e = e->next;
        } while (e != elast);
    }
    ...
    for (b = 1; b < nv-1; ++b)
    {
        ...
        do
        {
            if (e->end > b && degree[e->end] >= 4)
            {
                ...
                ec = eclast->next->next;
                eclast = eclast->prev;
                for (; ec != eclast; ec = ec->next) if (ISMARKED_V(ec->end)) return TRUE;
            }
            e = e->next;
        } while (e != elast);
    }
   ...
   purpose is to write the triangulation and to collect some stats. */
{
    ...
    if (oswitch) ADDBIG(nout_op[connec],doflip+1);

    if (pswitch)
    {
	if (oswitch) ADDBIG(nout_e_op[ne/2],doflip+1);
	ADDBIG(nout_e[ne/2],1);
    }

    if (polygonsize == 0)
    {
	if (oswitch) ADDBIG(nout_p_op[outside_face_size],doflip+1);
    ...
    if (needgroup && nbtot == 1) ADDBIG(ntriv,wt);
#ifdef STATS2
    num2 = 0;
...
static void
prune_poly_edgelist(EDGE *old[], int numold, EDGE *new[], int *numnew)

/* Copy from old[0..numold-1] to new[0..*numnew-1] each edge e with
   these two properties:
   1. Both ends of e have degree >= minpolydeg+1.
   2. e is contained in a 3-face.
   It is legal that old and new and &numold and numnew are the same.
*/
...
/* Finds all those loops so that after switching on one side of it there is
   ...
   The edges are given in min form.  It is assumed that the map handed to
   this routine does not contain vertices of valency 2 and the routine only
   returns edges that can be switched without producing some.

   Additionally it is checked that the double edges inside the one of the 
   triangles do not contain a loop on one side, since this would give a
   double edge in the dual. 
    ...
    for (i=0; i<num_loops; i++)
      { run=loops[i];
        ...
        if (ISMARKED_V(run->start)) /* already another loop started
          ...
          { 
	    if (ISMARKED(run->prev->prev) && (degree[run->prev->end]>3)
		...
		{ special_loop_makers[*num_spec_loops]=run->prev->min;
            ...
            if (ISMARKED(run->next->next) && (degree[run->next->end]>3)
...
/* Computes the lists of edges that can be switched to give a loop.
   ...
   For the computation it is useful to know where the double edges are.
   doubleedges[] must be a list of all edges contained in multiple edges
   (one direction only). "num_doubleedges" is its number;
...

{
    ...
    if (num_doubleedges<4) { *number_pos_loops=0; return; }
   ...
   There are no loops when this is called. */
{
    ...
    EDGE *loopmaker[MAXN],*loop[MAXN],**nb0,**nb1;
    int isloop[MAXN];
    long x,xlim,w;

    got_one(nbtot,nbop,2 + (numdoubles==0));
    if (numdoubles < 4) return;
    ...
    for (i = 0; i < nloopmakers; ++i) isloop[i] = FALSE;
    ...
    if (nbtot == 1)    /* Case of trivial group */
    {
        ...
        for (x = 1; x < xlim; ++x)
        {
            ...
            for (j = 0; j < nv; ++j) if (degree[j] < 3) break;
            if (j < nv) continue;
   ...
   *number the number of such things.  All edges are in min form. */
{
    ...
    {   run = firstedge[i];
        for (j=degree[i]; j; j--, run=run->next)
            if (run==run->min && degree[run->end]>=4)
            {
...
#ifdef POLY_CONN_TEST
    newconnec = POLY_CONN_TEST(edel,*connec);
#else
    newconnec = edge_del_conn(edel,*connec);
#endif
...
/* This routine begins the scan for general connected planar graphs formed
   by removing edges from 3-connected triangulations.  The current graph is
   such a triangulation, and the group is known (parameters nbtot,nbop).
*/
...
#else
    if (minpolyconnec == 3) scanpoly_c3(nbtot,nbop,feasible,nfeas,3,0,0);
    else                    scanpoly(nbtot,nbop,feasible,nfeas,3,0,0,3);
#endif
...
/* This routine receives a 3-conn triangulation with one more vertex
   that the output size.  It then makes the polygon triangulations by
...
/* Return 1 if there are at least vertices which are centres for
   a four-reduction (min4 case).  Return 0 otherwise.
*/
{
    EDGE *e;
    int v,count;
...
{
    int i,k;
    EDGE *e1,*e2,**nb,**nb0,**nblim,**nb1,**nb2;
...

{
    ...
    if (nv == maxnv)
    {
	got_one(nbtot,nbop,3);   /* Note connectivity is unknown */
	return;
    }
...

{
    ...
    if (nv == maxnv)
    {
	got_one(nbtot,nbop,2);   /* Note connectivity is really 2 or 3 */
	return;
    }
...
/* All the cases not handled in the other dispatch routines. */
{
    ...
    fprintf(msgfile," ");
    PRINTBIG(msgfile,nummindeg[4]);
    fprintf(msgfile," ");
    PRINTBIG(msgfile,nummindeg[5]);
    fprintf(msgfile,"\n");
#endif
...
{
    int startingsize,nbtot,nbop,i,hint,nbangles;
    EDGE *prevA[MAXN],*bangle[MAXE/2];
  ...
  /* Polytopes are made by first making 3-connected triangulations,
    ...
    CHECKRANGE(maxnv,"n",startingsize,MAXN);
    CHECKRANGE(maxnv,"n",startingsize,64);
    ...
    if (edgebound[1] < 0) edgebound[1] = 3*maxnv-6;
    if (edgebound[1] > 3*maxnv-6) edgebound[1] = 3*maxnv-6;
    if (maxfacesize <= 0) maxfacesize = MAXE;

    CHECKRANGE(edgebound[0],"-e",0,3*maxnv-6);
    CHECKRANGE(edgebound[1],"-e",0,3*maxnv-6);
    ...
    if (edgebound[0] == 3*maxnv-6 || maxfacesize == 3)
    {
	pswitch = FALSE;
...
/* Simple quadrangulations. */
{
    ...
    PERROR(minconnec==2&&xswitch,
           "-c2x is not implemented for quadrangulations");
...
main(int argc, char *argv[])
{
    ...
    if      (pswitch)                           polytope_dispatch();
    else if (polygonsize>=0)                    polygon_dispatch();
    else if (bswitch)                           bipartite_dispatch();
	...
	if (oswitch) SUMBIGS(totalout_op,nout_op[i]);
    }

    dosummary = 0; // Default is 1, change by Carlo Hamalainen
#ifdef SUMMARY
    nv = maxnv;
    SUMMARY();
#endif
